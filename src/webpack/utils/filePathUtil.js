...
module.exports = {
	...
	getHtmlFile: (srcPath) => {
		...
		return srcFiles.filter((item) => item.includes('.html')).map((item) => ({
			name: item,
			path: path.resolve(srcPath, item),
		}));
	},
	// Get the object of JS files from the specified path.
	getJsFile: (srcPath, jsDirectory = '', fileName, extensions = ['js'], ignoreFiles = []) => {
		const jsFileObj = {};
		const srcFiles = fs.readdirSync(path.join(srcPath, jsDirectory));
		srcFiles.filter((item) => !ignoreFiles.includes(item))
			.forEach((item) => {
				extensions.forEach((ext) => {
					let jsPath = '';
					if (item.includes(ext)) {
						if (item === `${fileName}.${ext}`) {
							jsPath = path.join(srcPath, jsDirectory, `${fileName}.${ext}`);
						}
					} else {
						jsPath = path.join(srcPath, jsDirectory, item, `${fileName}.${ext}`);
					}
